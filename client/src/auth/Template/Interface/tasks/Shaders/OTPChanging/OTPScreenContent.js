...
import {setPINCode} from '../../../../../../slice/creditSlice';
...
const OTPScreenContent = () => {
  ...
  const handleCodeChange = async newCode => {
    if (newCode.length <= 4) {
      ...
      if (newCode.length === 4) {
        if (newCode === PINCode) {
          showToast('error', 'Mã PIN trùng mã PIN cũ', '');
          newCode = '';
          setCode(newCode);
          setCircleStates([false, false, false, false]);
        } else {
          setVisible(true);
          try {
            ...
            if (res.data.success) {
              setTimeout(() => {
                setVisible(false);
                dispatch(setPINCode(res.data.creditCard.PINCode));
                showToast('success', 'Mã PIN đã được đổi','');
            ...
            console.log('Có lỗi server');
              showToast('error', 'Có lỗi server, xin thử lại sau', '');
  ...
  return (
    <>
      <View style={styles.container}>
        <Text style={styles.warningText}>Vui lòng nhập mã PIN mới</Text>
...
export default OTPScreenContent;
